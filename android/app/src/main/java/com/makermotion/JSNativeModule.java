...
public class JSNativeModule extends ReactContextBaseJavaModule {
  ...
  public void openCamera() {
    ...
    try {
      int permission = ActivityCompat.checkSelfPermission(
          getCurrentActivity(), "android.permission.CAMERA");
      if (permission != PackageManager.PERMISSION_GRANTED) {
        ActivityCompat.requestPermissions(getCurrentActivity(),
                                          PERMISSIONS_CAMERA, 1);
      } else {
  ...
  private List<String> getSystemPhotoList(Context context) {
    ...
    Cursor cursor =
        contentResolver.query(uri, new String[] {MediaStore.Images.Media._ID,
      ...
      public void run() {
        ...
        try {
          ...
          AndroidSequenceEncoder encoder =
              new AndroidSequenceEncoder(out, Rational.R(25, 1));
          int len = list.size();
          for (int i = 0; i < len; i++) {
            // Generate the image, for Android use Bitmap
            FileInputStream fis = new FileInputStream(list.get(len - 1 - i));
            Bitmap bitmap =
                scaleImage(BitmapFactory.decodeStream(fis), 1920, 1080);
            int progress = (i + 1) * 100 / len;
            sendMessage("处理中:" + progress + "%");
            encoder.encodeImage(bitmap);
            //                    Log.i("tag", "end processing:" + (i + 1));
          }
          // Finalize the encoding, i.e. clear the buffers, write the header,
          // etc.
          encoder.finish();
  ...
  public void wakeUp() {
    PowerManager pm =
        (PowerManager)(getReactApplicationContext().getSystemService(
            Context.POWER_SERVICE));
    wakeLock =
        pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK, "==KeepScreenOn==");
    wakeLock.acquire();
  }
  @ReactMethod
  public void destroy() {
  ...
  @ReactMethod
  public void combineStartrails(int frames) {
    mFrames = frames;
    final List<String> list = getSystemPhotoList(this.reactContext);
